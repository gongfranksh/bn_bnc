...
class proc_bnc_lead(models.TransientModel):
    ...
    def create_lead(self):
        ...
        records = self.env['pos.order'].search(
...
def proc_lead_buynow(self, memlist):
    #TODO 删除已有的crmlead记录
    sql = """
        delete from crm_lead where bnc_lead_id={0}
    """
    sql = sql.format(self.id)
    cr = self._cr
    cr.execute(sql)

    bnc_members = list(set(memlist))
    lead = self.env['bnc.lead'].search([('id', '=', self.id)])
    members = self.env['bnc.member'].search([('id', 'in', bnc_members)])
    lead_mem = []

    for mem in members:
        vals = {
            'name': lead['name'] + str(mem['strBncCardid']),
            'partner_id': mem['resid'].id,
            'date_action': lead['start'],
            'date_deadline': lead['end']
        }
        lead_mem.append((0, 0, vals))
