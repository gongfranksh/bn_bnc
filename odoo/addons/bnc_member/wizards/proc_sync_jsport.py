...
#        self.check_pos_data(startdate,para_interval)
        ...
        for (braid, saleid, memb_id,salerid,saleflag,saledate_order) in pos_order_list:
            ...
            for (saleman, saledate_detail,proid,SaleQty,NormalPrice,curprice,amount,SaleType,PosNo,profit) in pos_order_line:
                
                res.append((0,0,{
                    'product_id':self.env['product.template'].search_bycode(self._get_business()['strBuscode']+'-'+proid).id,
                    'price_unit':curprice,
                    'qty':SaleQty, 
    ...
    def proc_check_pos_data_weekly(self):
        ...
        for d in proc_date_task:
            if d['local_records_count'] <> d['remote_records_count']:
               ...
               _logger.info(d['proc_date'] +'====>'+'local have'+str(d['local_records_count'])+'==>remote have'+str(d['remote_records_count'])+'==>need to sync')
    ...
    def insert_pos_data_daily(self,begin,end):
        ...
        for (braid, saleid, memb_id,salerid,saleflag,saledate_order) in pos_order_list:
            ...
            for (saleman, saledate_detail,proid,SaleQty,NormalPrice,curprice,amount,SaleType,PosNo,profit) in pos_order_line:
                ...
                res.append((0,0,{
                    'product_id':self.env['product.product'].search([('default_code', '=',self._get_business()['strBuscode']+'-'+proid)]).id,
                    'price_unit':curprice,
                    'qty':SaleQty, 
                    'lngsaleid':self.env['hr.employee'].search_bycode(saleman).id,
                    }))
