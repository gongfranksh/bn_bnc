...
class proc_sync_jsport(models.TransientModel):
    ...
    def sync_product_class_all_branch(self):
        ...
        for db in self.env['bn.db.connect'].search([('bu_code', '=', self._get_business()['strBuscode'])]):
            list = self.get_product_class_recordset(bn_SQLCa(db),
                                                    self.env['res.company'].search_bycode(db['store_code']).id)
        ...
        for db in self.env['bn.db.connect'].search([('bu_code', '=', self._get_business()['strBuscode'])]):
            ...
            for d in proc_date_task:
                if d['local_records_count'] <> d['remote_records_count']:
                    print db['store_code'] + d['proc_date'] + '====>' + 'local have' + str(
                        d['local_records_count']) + '==>remote have' + str(
                        d['remote_records_count']) + '==>need to sync'
                    _logger.info(db['store_code'] + d['proc_date'] + '====>' + 'local have' + str(
                        d['local_records_count']) + '==>remote have' + str(
                        d['remote_records_count']) + '==>need to sync')
    ...
    def proc_check_pos_data_weekly(self):
        ...
        for d in proc_date_task:
            if d['local_records_count'] <> d['remote_records_count']:
                _logger.info("bnc=>buynow proc_check_pos_data_weekly" + d['proc_date'] + '====>' + 'local have' + str(
                    d['local_records_count']) + '==>remote have' + str(d['remote_records_count']) + '==>need to sync')
    ...
    def delete_duplicate_record_jspot(self):
        ...
        exec_sql = """ 
                ...
                in (select id from product_template where sale_ok=False)
                )
                """
        cr = self._cr
        cr.execute(exec_sql)
    ...
    def proc_sync_buynow_all(self):
        ...
        self.env['proc.sync.buynow'].delete_duplicate_record_jspot()
        return True
    ...
    def get_bn_pos_category_current_recordset(self, lens, store_code):
        ...
        if lens <> 0:
            select_sql = """ 
                      ...
                      """
            select_sql = select_sql.format(lens, self.env['res.company'].search_bycode(store_code).id,
                                           self._get_business()['id'])
        else:
            select_sql = """ 
                        select id,code from pos_category where code='{0}' and store_id={1} and buid={2} 
                      """
            select_sql = select_sql.format(store_code, self.env['res.company'].search_bycode(store_code).id,
                                           self._get_business()['id'])
        ...
        for (parentid, code) in vals:
            if lens <> 0:
                ...
                exec_sql = exec_sql.format(parentid, sublens, lens, code, self._get_business()['id'],
                                           self.env['res.company'].search_bycode(store_code).id)
            else:
                exec_sql = """ 
